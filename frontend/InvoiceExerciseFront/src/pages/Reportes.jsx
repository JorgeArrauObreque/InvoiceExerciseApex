...
import { getOverdueReport, getPaymentStatusSummary, getInconsistentReport } from './api';
...
export default function Reportes() {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      setIsLoading(true);
      try {
        const [overdue, summary, inconsistent] = await Promise.all([
          getOverdueReport(),
          getPaymentStatusSummary(),
          getInconsistentReport()
        ]);
        setReports({ overdueInvoices: overdue, statusSummary: summary, inconsistentInvoices: inconsistent });
      } catch (error) {
      ...
      <div className="bg-white p-6 rounded-lg shadow-md border border-slate-200">
        ...
        <ul className="divide-y divide-gray-200">
          {reports.overdueInvoices.map(inv => (
            <li key={inv.invoiceNumber} className="py-3 flex justify-between items-center">
              <div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md border border-slate-200">
        ...
        <ul className="divide-y divide-gray-200">
          {reports.inconsistentInvoices.map(inv => (
            <li key={inv.invoiceNumber} className="py-3">
